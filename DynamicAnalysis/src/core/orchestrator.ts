...
export class Orchestrator {
  ...
  private async compileTargets(): Promise<void> {
    ...
    await this.compileSanitizedBinaries();
    
    console.log("[*] Compiling non-ASAN binary for Eclipser...");
    await this.compileNoAsanBinary();
  }

  private async compileSanitizedBinaries(): Promise<void> {
    const sanitizers = [
      { type: 'asan', flags: ['-fsanitize=address'] },
      { type: 'ubsan', flags: ['-fsanitize=undefined'] },
      { type: 'msan', flags: ['-fsanitize=memory'] }
    ];

    for (const { type, flags } of sanitizers) {
      await this.compileSanitizedBinary(type, flags);
    }
  }

  private compileSanitizedBinary(sanitizer: string, extraFlags: string[]): Promise<void> {
    return new Promise((resolve, reject) => {
      const cFlags = ['-g', '-O0', '-fno-stack-protector', '-z', 'execstack'];
      const args = [...cFlags, ...extraFlags, '-o', `test_${sanitizer}.bin`, this.sourceFile];
      
      const compiler = this.getCompiler();
      const proc = spawn(
        join(this.aflPath, compiler),
        args,
        {
          cwd: this.aflMasterBox,
          stdio: 'inherit',
          env: { ...process.env, ...this.getSanitizerEnv(sanitizer) }
        }
      );
      ...
      proc.on('exit', code => {
        if (code === 0) {
          copyFileSync(
            join(this.aflMasterBox, `test_${sanitizer}.bin`),
            join(this.aflSlaveBox, `test_${sanitizer}.bin`)
          );
          resolve();
        } else {
          reject(new Error(`${sanitizer} build failed with code ${code}`));
        }
      });
    });
  }

  private getSanitizerEnv(sanitizer: string): NodeJS.ProcessEnv {
    switch (sanitizer) {
      case 'asan': return { AFL_USE_ASAN: '1' };
      case 'ubsan': return { AFL_USE_UBSAN: '1' };
      case 'msan': return { AFL_USE_MSAN: '1' };
      default: return {};
    }
  }
  ...
  // Returns paths to all compiled binaries
  public getAllBinaryPaths(): { path: string; type: string }[] {
    const binaries = [];
    const sanitizers = ['asan', 'ubsan', 'msan'];
    
    for (const type of sanitizers) {
      const binPath = join(this.aflMasterBox, `test_${type}.bin`);
      if (fs.existsSync(binPath)) {
        binaries.push({ path: binPath, type });
      }
    }
  ...
  private getCompiler(): string {
    ...
    return ext === '.c' ? 'afl-clang-fast' : 'afl-clang-fast++';
  }
