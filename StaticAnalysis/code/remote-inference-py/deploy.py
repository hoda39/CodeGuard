...
def predict_gpu(request: Request):
    ...
    if not functions:
        ...
        # Run the model
        result = main(functions, True)
        # Map vulnerabilities to lines
        enriched_results = []
        for idx, func_code in enumerate(functions):
            lines = func_code.split('\n')
            line_scores = result['batch_line_scores'][idx] if 'batch_line_scores' in result else []
            # Find the line with the highest score (most likely vulnerable)
            if line_scores:
                max_score = max(line_scores)
                vuln_line = line_scores.index(max_score) + 1  # 1-based line number
            else:
                vuln_line = 1
            enriched_results.append({
                'vulnerable': bool(result['batch_vul_pred'][idx]) if 'batch_vul_pred' in result else False,
                'probability': float(result['batch_vul_pred_prob'][idx]) if 'batch_vul_pred_prob' in result else 0.0,
                'line': vuln_line,
                'message': 'Potential vulnerability detected',
                'source': '\n'.join(lines),
            })
        return json.dumps({'vulnerabilities': enriched_results})
